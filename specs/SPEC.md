...
- Pain score and pain-state controls. The pain subsection heading remains in the markup for assistive technologies but is visually hidden. The pain score field is labeled `痛みスコア`, and the pain-state field is labeled `痛みの状態`.
...
- The pain record button remains disabled while the pain state is unselected.
- Selecting a pain state enables the pain record button. After a pain record is saved and the input is reset, the button is disabled again.

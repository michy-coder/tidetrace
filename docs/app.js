...
function normalizeImportedData(data) {
  if (data && data.settings && typeof data.settings === 'object' && !Array.isArray(data.settings)) {
    data.settings.healthReviewColumns = normalizeHealthReviewColumns(data.settings);
  }
  return data;
}
...
function loadStoredData() {
  ...
  if (normalizedText !== stored) localStorage.setItem(STORAGE_KEY, normalizedText);
  return normalized;
}
...
function completeInitialSetup() {
  ...
  appData.settings.healthReviewColumns = defaultHealthHistoryColumns(appData.settings);
  clearSaveFeedback();
  saveData();
  showApp();
}
...
function defaultHealthHistoryColumns(settings) { const used = new Set(['日付'].map(normalizeColumnLabel)); const cols = [ ['tidetrace:pain:max','最大'], ['tidetrace:pain:average','平均'] ].map(([columnId, shortLabel]) => ({ columnId, shortLabel, shortLabelMode: 'auto' })); cols.forEach((c) => used.add(normalizeColumnLabel(c.shortLabel))); { const meds = sortedMedicationOptions(((settings && settings.medicationOptions) || []).filter((o) => o.active)); const labels = healthHistoryMedicationLabels(meds); meds.forEach((option, index) => { cols.push({ columnId: `tidetrace:medication:${option.id}:count`, shortLabel: labels[index], shortLabelMode: 'auto' }); used.add(normalizeColumnLabel(labels[index])); }); } ['steps','sleep-duration','sleep-bpm','sleep-hrv','wake-hrv'].forEach((id) => { const m = HEARTWATCH_CATALOG.find((x) => x.id === id); cols.push({ columnId: m.columnId, shortLabel: m.defaultShortLabel, shortLabelMode: 'auto' }); used.add(normalizeColumnLabel(m.defaultShortLabel)); }); return cols; }
function normalizeHealthReviewColumns(settings) { const safeSettings = settings && typeof settings === 'object' ? settings : {}; const source = Array.isArray(safeSettings.healthReviewColumns) ? safeSettings.healthReviewColumns : defaultHealthHistoryColumns(safeSettings); const seen = new Set(); const out = []; source.forEach((item) => { if (!item || typeof item !== 'object' || typeof item.columnId !== 'string' || item.columnId === 'date' || seen.has(item.columnId)) return; const metric = healthHistoryMetricById(item.columnId, safeSettings); if (!metric && !/^tidetrace:(medication|pain-state):/.test(item.columnId)) return; seen.add(item.columnId); out.push({ columnId: item.columnId, shortLabel: String(item.shortLabel || (metric && metric.defaultShortLabel) || '').trim(), shortLabelMode: item.shortLabelMode === 'custom' ? 'custom' : 'auto' }); }); if (!out.length) return defaultHealthHistoryColumns(safeSettings); return refreshAutomaticHealthHistoryLabels(out, safeSettings); }
function refreshAutomaticHealthHistoryLabels(columns, settings) { const used = new Set(); return columns.map((item) => { const metric = healthHistoryMetricById(item.columnId, settings); let shortLabel = String(item.shortLabel || '').trim(); if (!shortLabel) shortLabel = metric ? metric.defaultShortLabel : shortLabel; const normalized = normalizeColumnLabel(shortLabel); if (item.shortLabelMode !== 'custom' && (used.has(normalized) || !shortLabel)) shortLabel = metric ? metric.generateShortLabel(used) : uniqueGeneratedLabel('項目', used); else used.add(normalized); return { columnId: item.columnId, shortLabel, shortLabelMode: item.shortLabelMode === 'custom' ? 'custom' : 'auto' }; }); }
function ensureHealthReviewColumns(settings) { settings.healthReviewColumns = normalizeHealthReviewColumns(settings); return settings.healthReviewColumns; }
function healthHistoryMetricById(columnId, settings) { return healthHistoryMetricCatalog(settings).find((m) => m.columnId === columnId) || null; }
function healthHistoryMetricCatalog(settings) { const safeSettings = settings && typeof settings === 'object' ? settings : {}; const metrics = [ { columnId: 'tidetrace:pain:max', type: 'pain', kind: 'max', fullName: '日ごとの痛み 最大', defaultShortLabel: '最大', category: 'TideTrace', subsection: '日ごとの痛み' }, { columnId: 'tidetrace:pain:average', type: 'pain', kind: 'average', fullName: '日ごとの痛み 平均', defaultShortLabel: '平均', category: 'TideTrace', subsection: '日ごとの痛み' }, { columnId: 'tidetrace:pain:min', type: 'pain', kind: 'min', fullName: '日ごとの痛み 最小', defaultShortLabel: '最小', category: 'TideTrace', subsection: '日ごとの痛み' }, { columnId: 'tidetrace:pain:count', type: 'pain', kind: 'count', fullName: '日ごとの痛み 記録数', defaultShortLabel: '痛回', category: 'TideTrace', subsection: '日ごとの痛み' }, { columnId: 'tidetrace:note:count', type: 'note', fullName: '単独メモ数', defaultShortLabel: 'メモ', category: 'TideTrace', subsection: '薬・メモ' } ]; sortedPainOptions(safeSettings.painStateOptions || []).forEach((option) => ['max','average','count'].forEach((kind) => metrics.push({ columnId: `tidetrace:pain-state:${option.id}:${kind}`, type: 'pain-state', kind, option, fullName: `${option.label}${option.active ? '' : '（非表示中）'} ${kind === 'max' ? '最大' : kind === 'average' ? '平均' : '回数'}`, defaultShortLabel: healthHistoryPainStateLabel(option, kind, new Set()), generateShortLabel: (used) => healthHistoryPainStateLabel(option, kind, used), category: 'TideTrace', subsection: '状態別の痛み' }))); sortedMedicationOptions(safeSettings.medicationOptions || []).forEach((option) => metrics.push({ columnId: `tidetrace:medication:${option.id}:count`, type: 'medication', option, fullName: `${option.label}${option.active ? '' : '（非表示中）'} 服薬回数`, defaultShortLabel: healthHistoryMedicationLabel(option, new Set()), generateShortLabel: (used) => healthHistoryMedicationLabel(option, used), category: 'TideTrace', subsection: '薬・メモ' })); HEARTWATCH_CATALOG.forEach((h) => metrics.push({ ...h, fullName: h.fullName, defaultShortLabel: h.defaultShortLabel, generateShortLabel: (used) => uniqueGeneratedLabel(h.defaultShortLabel, used) })); return metrics; }
function selectedHealthHistoryColumns() { ensureHealthReviewColumns(appData.settings); return [HEALTH_HISTORY_DATE_COLUMN, ...appData.settings.healthReviewColumns.map((item) => { const metric = healthHistoryMetricById(item.columnId, appData.settings); return metric ? { ...metric, shortLabel: item.shortLabel } : null; }).filter(Boolean)].map((column) => ({ ...column, label: column.shortLabel })); }
...
function healthHistoryColumnEditorName(metric) {
  if (!metric) return '';
  if (metric.type === 'pain') {
    return { max: '痛み 最大', average: '痛み 平均', min: '痛み 最小', count: '痛み 回数' }[metric.kind] || metric.fullName;
  }
  if (metric.type === 'pain-state') {
    const stateName = `${metric.option?.label || ''}${metric.option?.active ? '' : '（非表示中）'}`;
    return `${stateName} ${metric.kind === 'max' ? '最大' : metric.kind === 'average' ? '平均' : '回数'}`.trim();
  }
  if (metric.type === 'medication') {
    return `${metric.option?.label || ''}${metric.option?.active ? '' : '（非表示中）'} 回数`.trim();
  }
  return metric.fullName;
}

function renderHealthHistoryColumnEditor() {
  const root = $('health-history-columns-editor');
  if (!root) return;
  if (!healthHistoryColumnDraft) healthHistoryColumnDraft = normalizeHealthReviewColumns(appData.settings).map((x) => ({ ...x }));
  const selectedIds = new Set(healthHistoryColumnDraft.map((x) => x.columnId));
  const metrics = healthHistoryMetricCatalog(appData.settings);
  const rowHtml = healthHistoryColumnDraft.map((item, index) => {
    const metric = healthHistoryMetricById(item.columnId, appData.settings);
  ...
  root.innerHTML = `<div class="fixed-date-column">日付（先頭固定）</div>${rowHtml || '<p class="empty">表示項目がありません。</p>'}<p id="health-history-columns-error" class="message error"></p><div class="health-column-add-section"><h4>項目を追加</h4><details><summary>TideTrace</summary><h5>日ごとの痛み</h5><div class="health-column-checks">${painKinds}</div><h5>状態別の痛み</h5>${states}<h5>薬・メモ</h5><div class="health-column-checks">${meds}</div></details>${cats}</div><div class="health-column-buttons"><button id="save-health-history-columns" type="button" class="button-base button-full primary-button">保存</button><button id="reset-health-history-columns" type="button" class="button-base button-full secondary-button">初期表示に戻す</button></div>`;
}
